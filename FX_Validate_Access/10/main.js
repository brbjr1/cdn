...
function DescribeAllSObjects(conn, callback)
{
    GetAllCustomObject(conn, function(err, fullNames)
    {
        ...
        {
            var results = [];
            DescribeAllSObjects2(conn, results, 1, '', fullNames, function(err, result)
            {
                if (err && !result)
                {
                    callback(err, null);
...
function GetSpanPermission(hasaccess, ispermissionable, grantedby, CusomObject, sftype)
{
    ...
    results += ' <span class="glyphicon ' + (hasaccess == true ? 'glyphicon-check dropdown-toggle' : 'glyphicon-unchecked') + ' ' + (ispermissionable == false ? 'text-grey' : '') + '" type="button" data-toggle="dropdown"></span>';
    if (hasaccess == true)
    {
...
/*
function GetSpanPermission2(hasaccess, ispermissionable, grantedby, CusomObject, sftype)
{
    var results = '';
