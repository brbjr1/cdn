     ...
     //  alert(mycdnurl);
        ...
        function ShowFilePermissionsModal(sobject, sobjectlabel)
        {
            ...
            {
                datatableFieldPermissionsresult += '<table id="FieldPermissionsTable" class="display" style="display:none" cellspacing="0" width="100%"><thead><tr>';
                ...
                function(index, d) {
                    ...
                    datatableFieldPermissionsresult += '<td>'+GetSpanPermission(d.HasRead,d.ReadIsAssignable,d.HasReadGrantedBy)+'</td>'
                    datatableFieldPermissionsresult += '<td>'+GetSpanPermission(d.HasEdit,d.EditIsAssignable,d.HasEditGrantedBy)+'</td>'
                    datatableFieldPermissionsresult += '</tr>';
                });
        ...
        function CurrentUserHasModifyAllDataAccess(conn, callback)
        {
            ...
            QueryRecords(conn,myquery,function(UserQueryResults)
            {
                if (UserQueryResults.error)
                {
                    callback('Error:' + UserQueryResults.error,null)
                }
                var myUser = UserQueryResults.results[0];
                ...
                else
                {
                    ...
                    QueryRecords(conn,myquery,function(PermissionSetQueryResults)
                    {
                        if (PermissionSetQueryResults.error)
                        {
                            callback('Error:' + PermissionSetQueryResults.error,null)
                        }
                        else
                        {
                            //console.log(PermissionSetQueryResults);
        ...
        function SetObjectResult(mypermission, fresult, proPermName, IsProfile)
        {
            try 
            {
                ...
                for (var i2=0; i2 < fresult.FXObjects.length; i2++) 
                {
                    ...
                    if (myresultobject.APIName == sobjectname)
                    {
                        ...
                        if (mypermission.allowCreate == 'true')
                        {
                            ...
                            myresultobject.HasCreateGrantedBy.push({Id:'123',IsProfile:IsProfile,Name:proPermName});

                        }
                        if (mypermission.allowDelete == 'true')
                        {
                            myresultobject.HasDelete = true;
                            myresultobject.HasDeleteGrantedBy.push({Id:'123',IsProfile:IsProfile,Name:proPermName});
                        }
                        if (mypermission.allowEdit == 'true')
                        {
                            myresultobject.HasEdit = true;
                            myresultobject.HasEditGrantedBy.push({Id:'123',IsProfile:IsProfile,Name:proPermName});
                        }
                        if (mypermission.allowRead == 'true')
                        {
                            myresultobject.HasRead = true;
                            myresultobject.HasReadGrantedBy.push({Id:'123',IsProfile:IsProfile,Name:proPermName});
                        }
                        if (mypermission.modifyAllRecords == 'true')
                        {
                            myresultobject.HasModifyAll = true;
                            myresultobject.HasModifyAllGrantedBy.push({Id:'123',IsProfile:IsProfile,Name:proPermName});
                        }
                        if (mypermission.viewAllRecords == 'true')
                        {
                            myresultobject.HasViewAll = true;
                            myresultobject.HasViewAllGrantedBy.push({Id:'123',IsProfile:IsProfile,Name:proPermName});
                        }
                        break;
                    }
                }
        ...
        function SetFeildResult(mypermission, fresult, proPermName, IsProfile)
        {
            try 
            {
        ...
        function AddZipContentsToHashTableAsJson(data, callback)
        {
            ...
            new_zip.loadAsync(data, {base64:true})
            .then(function(zip) 
            {
        ...
        function getfieldResults(item)
        {
            ...
            for (var i=0; i < myfields.length; i++) 
            {
                ...
                var fieldresult = {EditIsAssignable:EditIsAssignable,HasEdit:HasEdit,HasEditGrantedBy:[],HasRead:HasRead,HasReadGrantedBy:[],Label:flabel,Name:field.fullName,ReadIsAssignable:ReadIsAssignable,autoNumber:field.type == 'AutoNumber',calculated:field.formula != undefined,permissionable:true};
        ...
        {   
            ...
            if (result.FXObjects != undefined) 
            {
                ...
                function(index, d) {
                    ...
                    datatableFXObjectsresult += '<td>' + d.Label + '</td>';
                    datatableFXObjectsresult += '<td>' + d.APIName + '</td>';
                    datatableFXObjectsresult += '<td>'+GetSpanPermission(d.HasRead,true,d.HasReadGrantedBy)+'</td>'
                    datatableFXObjectsresult += '<td>' + GetSpanPermission(d.HasCreate,true,d.HasCreateGrantedBy) + '</td>';
                    datatableFXObjectsresult += '<td>' + GetSpanPermission(d.HasEdit,true,d.HasEditGrantedBy) + '</td>';
                    datatableFXObjectsresult += '<td>' + GetSpanPermission(d.HasDelete,true,d.HasDeleteGrantedBy) + '</td>';
                    datatableFXObjectsresult += '<td>' + GetSpanPermission(d.HasViewAll,true,d.HasViewAllGrantedBy) + '</td>';
                    datatableFXObjectsresult += '<td>' + GetSpanPermission(d.HasModifyAll,true,d.HasModifyAllGrantedBy) + '</td>';
                    datatableFXObjectsresult += '<td><a onclick="ShowFilePermissionsModal(&apos;' + d.APIName  +  '&apos;,&apos;' + d.Label  +  '&apos;);">' + getcount(d.fields,true,false,false) + '</a></td>';
                    datatableFXObjectsresult += '<td><a onclick="ShowFilePermissionsModal(&apos;' + d.APIName  +  '&apos;,&apos;' + d.Label  +  '&apos;);">' + getcount(d.fields,false,true,false) + '</a></td>';
                    datatableFXObjectsresult += '<td><a onclick="ShowFilePermissionsModal(&apos;' + d.APIName  +  '&apos;,&apos;' + d.Label  +  '&apos;);">' + getcount(d.fields,false,false,true) + '</a></td>';
                    datatableFXObjectsresult += '</tr>';
                });
                datatableFXObjectsresult += '</tbody>';
            ...
            if (result.FXRelatedObjects != undefined) 
            {
                ...
                function(index, d) {
                    ...
                    datatableFXRelatedObjectsresult += '<td>' + GetSpanPermission(d.HasRead,true,d.HasReadGrantedBy)+'</td>'
                    datatableFXRelatedObjectsresult += '<td>' + GetSpanPermission(d.HasCreate,true,d.HasCreateGrantedBy) + '</td>';
                    datatableFXRelatedObjectsresult += '<td>' + GetSpanPermission(d.HasEdit,true,d.HasEditGrantedBy) + '</td>';
                    datatableFXRelatedObjectsresult += '<td>' + GetSpanPermission(d.HasDelete,true,d.HasDeleteGrantedBy) + '</td>';
                    datatableFXRelatedObjectsresult += '<td>' + GetSpanPermission(d.HasViewAll,true,d.HasViewAllGrantedBy) + '</td>';
                    datatableFXRelatedObjectsresult += '<td>' + GetSpanPermission(d.HasModifyAll,true,d.HasModifyAllGrantedBy) + '</td>';
                    datatableFXRelatedObjectsresult += '<td><a onclick="ShowFilePermissionsModal(&apos;' + d.APIName  +  '&apos;,&apos;' + d.Label  +  '&apos;);">' + getcount(d.fields,true,false,false) + '</a></td>';
                    datatableFXRelatedObjectsresult += '<td><a onclick="ShowFilePermissionsModal(&apos;' + d.APIName  +  '&apos;,&apos;' + d.Label  +  '&apos;);">' + getcount(d.fields,false,true,false) + '</a></td>';
                    datatableFXRelatedObjectsresult += '<td><a onclick="ShowFilePermissionsModal(&apos;' + d.APIName  +  '&apos;,&apos;' + d.Label  +  '&apos;);">' + getcount(d.fields,false,false,true) + '</a></td>';
                    datatableFXRelatedObjectsresult += '</tr>';
                });
                datatableFXRelatedObjectsresult += '</tbody>';
            ...
            if (result.ApexClassAccess != undefined) 
            {
                ...
                function(index, d) {
                    ...
                    datatableApexClassesresult += '<td>'+GetSpanPermission(d.HasAccess,true,d.GrantedBy)+'</td>'
                    datatableApexClassesresult += '</tr>';
                });
            ...
            if (result.VFPageAccess != undefined) 
            {
                ...
                function(index, d) {
                    ...
                    datatableVisualforcePagesresult += '<td>'+GetSpanPermission(d.HasAccess,true,d.GrantedBy)+'</td>'
                    datatableVisualforcePagesresult += '</tr>';
                });
            ...
            if (result.SystemPermissions != undefined) 
            {
                ...
                function(index, d) {
                    ...
                    datatableSystemPermissionsresult += '<td>'+GetSpanPermission(d.HasAccess,true,d.GrantedBy)+'</td>'
                    datatableSystemPermissionsresult += '</tr>';
                });
        ...
        function DocheckRetrieveStatus(conn, id,callback) 
        {
            conn.metadata.checkRetrieveStatus (id, function(err, result) 
            {
                ...
                else if (result.done == 'false')
                {
                    ...
                    setTimeout(function() {DocheckRetrieveStatus(conn,id, callback);}, 5000);
                }
        ...
        {
            var lstquery=[{type:'CustomObject'}];
        ...
        {
            var results = [], length = Math.ceil(array.length / unit);
        ...
        function GetSpanPermission(hasaccess, ispermissionable, grantedby )
        {
            var results = '';
            ...
            if(hasaccess == true)
            {
                ...
                if (gresults != '')
                {
                    results += 'data-toggle="popover" title="Assigned From" data-trigger="hover"  data-content="' + gresults + '"';
                }
            }
